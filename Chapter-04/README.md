...
Let us check the Example 15. Likewise for an object, array, if an array 'arr' is passed as an argument of a method 'foo', foo(arr) makes a change to the elements of 'arr', but not to 'arr' variable itself. A method 'bar' does not alter anything for 'arr', since it tries to touch only a new object int[ ]{666666, 7777777, 22}, being referred by a copied reference 'y'.
